...
본 시스템은 예측 모델로부터 전력 빅데이터 기반 신사업모델을 제시한다. 기존 데이터 제공에만 머무르던 수동적인 서비스에서 나아가, 실시간 예측을 제공하는 능동적인 서비스를 제공한다. 가령 소비자는 서비스 형태의 '가격 예측 모델'을 통해 세탁기를 가장 싼 전기 요금 시간대에 맞춰 작동하고, 한낮에 에어컨 전력 사용을 조절하는 등의 자율적 에너지 사용이 가능하다. 공급자는 서비스 형태의 '수요 예측 모델'을 통해 공급 예비율과 설비 예비율을 조정할 수 있다. 사업자는 서비스 형태의 '전력 공급망 효율 계산 모델'을 통해 발전소 건설, 마이크로 그리드 구축 등을 행할 수 있다.
...
궁극적으로 본 시스템은 실시간 전력망 통합관제 시스템, P2P 에너지 거래, 서비스 형태의 추론으로부터 서비스 형태의 스마트 그리드(SGaaS, Smart-Grid-as-a-Service)를 제공한다. 이는 전력망과 독립적인 마이크로 그리드 형성을 보조한다. 유니스왑을 통해 공급자-소비자 간 거래를 넘어 마이크로 그리드 간 자동화된 가치 교환, 전력 교환이 가능하다. 나아가 스마트 시티, V2X(Vehicle to Everything) 등 이종 사업부문에서 서비스 형태의 스마트 그리드를 활용할 수 있다. 
