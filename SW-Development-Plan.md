...
제안하는 SW는 플라즈마(Plasma), PPDL(Privacy-Preserving Deep Learning), 유니스왑(Uniswap) 프로토콜을 구현하고 이들을 릴레이(Relay) 등의 방법으로 연동해 하나의 시스템을 구축했다. 이렇게 구축된 시스템은 높은 처리량과 확장성을 가지는 블록체인이며, 또한 프라이버시를 보장할 수 있는 익명화 시스템이다. 본 시스템 상에서는 여러 스마트 컨트랙트(smart contract)가 구동되며, 이를 통해 P2P 거래와 추론 서비스를 제공한다.
...
유니스왑 프로토콜상에서 ETH를 토큰으로 교환하는 거래는 다음과 같은 절차를 통해 이뤄진다.
* 현재 상태(State 1)에서는 ETH 풀에 10 ETH가, 토큰(MG1) 풀에 500 MG1이 들어있다. 이 두 수량의 곱은 Invariant 또는 Constant Product라고 하며, 항상 일정하게 유지되어야 하는 값이다. 본 예시에서는 5000에 해당한다.
* 구매자는 교환하고자 하는 1 ETH를 ETH 풀에 전송한다.
* 다음 상태(State 2)에서 ETH 풀의 값이 11 ETH로 조율되므로, Constant Product인 5000을 유지하기 위해 MG1 풀이 454.5로 조정된다.
* MG1 풀은 State 1에서 500의 값을 가졌으나 현재 State 2에서 454.5로 조율되어야 하므로, 구매자는 둘의 차액인 45.5 MG1을 수령한다.

만일 구매자가 더 많은 양의 ETH를 MG1으로 교환하고자 할 경우 Constant Product의 특성상 '1 ETH당 MG1 교환 수량'은 떨어지게 된다. 따라서 단기간 대량 교환이 어려워 시장 교란 행위를 방지할 수 있다.

본 SW는 전력을 토큰화하여 풀에서 보유하고, ETH 풀과의 상호작용을 통해 P2P 거래를 제공한다. 특히 분산전원이나 에너지저장장치 등에 저장된 잉여 전력에 대응되는 토큰은 유니스왑 거래 즉시 전송받아 활용할 수 있으므로, 에너지 거래의 속도와 효용성을 높인다. 

#### Token to Token

![t2t](https://github.com/twodude/Energy-X-Security-Hackathon/blob/master/images/uniswap_t2t.png)

마이크로 그리드 간의 에너지 거래는 토큰-토큰의 형태를 보인다. 이는 ETH-토큰 간 거래, 토큰-ETH 간 거래를 연이어 수행하는 하나의 트랜잭션 호출로 수행할 수 있다.
...
블록체인 기반 P2P 거래와 실시간 전력망에 기반해 소비자는 가장 값싼 전기를 탐색 구매할 수 있으며, 공급자는 잉여 전력 유지보수 비용을 절감할 수 있다. 또한 중간 거래 비용을 획기적으로 줄일 수 있다. 통합된 유니스왑 프로토콜은 에너지 거래에 대한 유동성 및 신뢰성을 높여, P2P 에너지 거래에 회의적인 사용자에게도 유인책을 제공한다.
