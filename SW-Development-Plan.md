...
본 시스템의 거래는 zk-ERC20로 명명하는 익명화 토큰에 기반한다. 영지식증명 기술로 거래 참여자와 내역 숨겨 거래를 완전히 익명화할 수 있다. 또한 본 시스템은 PPDL(Privacy-Preserving Deep Learning)을 적용해 종래의 스마트 전력 계량기 등으로부터 발생할 수 있는 데이터 프라이버시 문제를 해결한다. 이를 통해 공급자와 소비자 양 측의 프라이버시를 보장함으로써 개인정보 유출의 우려를 잠식, 마켓 참여의 장벽을 크게 낮춘다.
